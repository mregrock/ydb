...
{% include [tags](./_includes/tags_list.md) %}

## host_configs — типовые конфигурации хостов {#host-configs}

{% include notitle [static_parameter-tag](./_includes/tags.md#static_parameter) %}
...
Топология кластера задается в качестве параметра `erasure` в разделе `config` и принимает одно из значений, указанных в таблице выше.
...
```yaml
security_config:
  # флаги, влияющие на настройку безопасности при первом старте кластера
...
Параметры безопасности можно задать в разделе [`domains_config`](#domains-config), однако этот вариант является устаревшим и не рекомендуется для использования.
...
Значение по умолчанию: `false`.
    ||
|| `disable_builtin_groups` | Отказаться от создания [встроенных групп](../../security/builtin-security.md), даже если явные группы по умолчанию ([`security_config.default_groups`](#security)) не заданы.
...
Не требуется для работы кластера с [автоматической конфигурацией](../../devops/configuration-management/configuration-overview.md) State Storage и статической группы.
...
{% include notitle [dynamic_parameter-tag](./_includes/tags.md#dynamic_parameter) %}
